...
const createOne: ServerRoute = {
  ...
  handler: async (req, h) => {
    ...
    return h.response(`Employee "${firstname} ${lastname}" successfully created.`).code(201)
  },
}
...
const updateOne: ServerRoute = {
  ...
  handler: async (req, h) => {
    const { firstname, lastname, title, salary, startDate, endDate } = req.payload as IEmployee
    const { id } = req.params
...
const assignProject: ServerRoute = {
  ...
  handler: async (req, h) => {
    ...
    await h.context.bindModelOneToMany(Employee, 'project', id, projectId)
